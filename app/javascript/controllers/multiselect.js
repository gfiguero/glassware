...
export default class Multiselect extends Controller {
  ...
  connect() {
    ...
    if (this.selectedValue.length) this.selectedValueInitialize()
  ...
  async selectedValueInitialize() {
    const response = await fetch(this.preloadUrlValue + '?' + new URLSearchParams(this.selectedValue.map(i=>['items[]', i])).toString())
    this.selectedValue = await response.json()
